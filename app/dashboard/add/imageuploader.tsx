  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    const uploadPromises = toUpload.map(async (file, index) => {
      ...
      try {
        ...
        setImages((prev) =>
          prev.map((it) => (it.id === itemId ? { ...it, progress: 20 } : it))
        );
        ...
        setImages((prev) =>
          prev.map((it) => (it.id === itemId ? { ...it, progress: 60 } : it))
        );
        ...
        setImages((prev) =>
          prev.map((it) => (it.id === itemId ? { ...it, progress: 90 } : it))
        );
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-2 mt-2">
              <button
                ...
                disabled={!canAddMore}
                className={`w-full sm:w-auto px-4 py-2 rounded-lg text-sm transition-colors duration-200 ${
                  canAddMore
                    ? "bg-white dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300"
              ...
              <button
                ...
                disabled={!canAddMore}
                className={`flex items-center gap-2 w-full sm:w-auto px-4 py-2 rounded-lg text-sm transition-colors duration-200 ${
                  canAddMore
                    ? "bg-white dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"
                    : "cursor-not-allowed opacity-50"
                }`}
              >
